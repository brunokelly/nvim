return {
  ...
  config = function()
    ...
    mason_lspconfig.setup({
      ensure_installed = { "gopls", "lua_ls", "html", "emmet_ls", "graphql", "templ" },
    ...
    vim.api.nvim_create_autocmd("LspAttach", {
      ...
      callback = function(ev)
        ...
        opts.desc = "Show LSP type defs";      keymap.set("n", "gt", "<cmd>Telescope lsp_type_definitions<CR>", opts)
        opts.desc = "Code actions";            keymap.set({ "n", "v" }, "<leader>ca", vim.lsp.buf.code_action, opts)
        opts.desc = "Rename";                   keymap.set("n", "<leader>rn", vim.lsp.buf.rename, opts)
        opts.desc = "Diagnostics (file)";      keymap.set("n", "<leader>D", "<cmd>Telescope diagnostics bufnr=0<CR>", opts)
        opts.desc = "Line diagnostic";         keymap.set("n", "<leader>d", vim.diagnostic.open_float, opts)
        ...
        opts.desc = "Hover";                   keymap.set("n", "K", vim.lsp.buf.hover, opts)
    ...
    -- Ícones dos diagnostics
    local signs = { Error = " ", Warn = " ", Hint = "󰠠 ", Info = " " }
    for t, ic in pairs(signs) do
      local hl = "DiagnosticSign" .. t
      vim.fn.sign_define(hl, { text = ic, texthl = hl, numhl = "" })
    end

    -- Opcional: como os diagnostics aparecem
